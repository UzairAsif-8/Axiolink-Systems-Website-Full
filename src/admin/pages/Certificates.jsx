...
const CertificatesAdmin = () => {
  ...
  const handleIssue = () => {
    const code = formatCertificateCode(form.certificateCode);
    if (!isCompleteCertificateCode(code)) {
      toast.error(`Certificate code must be ${CERT_CODE_PLACEHOLDER}`);
      return;
    }
    if (!form.studentName.trim() || !form.courseName.trim()) {
      toast.error("Student and course are required");
      return;
    }
    issueMut.mutate({ ...form, certificateCode: code });
  };
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 p-6 shadow-sm">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Input
            ...
            onChange={(e) =>
              setForm({ ...form, certificateCode: formatCertificateCode(e.target.value) })
            }
        ...
        <Button onClick={handleIssue} loading={issueMut.isPending} className="mt-4">Issue Certificate</Button>
      </div>
                ...
                (data || []).map((c) => (
                  <tr key={c.id}>
                    <td className="px-6 py-4 font-mono text-xs">
                      {displayCertificateCode(c.certificateCode)}
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-xl border p-6 shadow-xl space-y-4">
            ...
            <Input
              label="Code"
              placeholder={CERT_CODE_PLACEHOLDER}
              value={formatCertificateCode(editTarget.certificateCode)}
              onChange={(e) =>
                setEditTarget({
                  ...editTarget,
                  certificateCode: formatCertificateCode(e.target.value),
                })
              }
              className="font-mono tracking-wide"
              autoComplete="off"
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button
                ...
                onClick={() => {
                  const code = formatCertificateCode(editTarget.certificateCode);
                  if (!isCompleteCertificateCode(code)) {
                    toast.error(`Certificate code must be ${CERT_CODE_PLACEHOLDER}`);
                    return;
                  }
                  updateMut.mutate({
                    id: editTarget.id,
                    body: { ...editTarget, certificateCode: code },
                  });
                }}
              >
                Save
